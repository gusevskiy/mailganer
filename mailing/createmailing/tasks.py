...
def order_created(mailing_id):
    ...
    for mail in data.mailing_emails.all():
        ...
        mailing_log = MailingEmails.objects.create(mailing=data,
                                                   email=mail.email)

        tracking_url_subscribed = "{}{}".format(
            settings.SITE_URL,
            reverse('createmailing:track_email_subscribed',
                    args=[mailing_log.tracking_id]))
        # экранирование спец символов в url
        tracking_url_subscribed = escape(tracking_url_subscribed)

        tracking_url_unsubscribed = "{}{}".format(
            settings.SITE_URL,
            reverse('createmailing:track_email_unsubscribed',
                    args=[mailing_log.tracking_id]))
        # экранирование спец символов в url
        ...
        send_mail(subject=data.header_email,
